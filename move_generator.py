...
def find_caps(state, r, c, path, result):
    ...
    if pc.is_dama:
        ...
        dirs = [(-1, -1), (-1, 1)] if pc.player == 0 else [(1, -1), (1, 1)]
    ...
    for dr, dc in dirs:
        ...
        if (mp and mp.player != pc.player
                and not lp
                and (lr, lc) not in path):
            find_caps(state, lr, lc, path + [(lr, lc)], result)
            found = True
...
def get_walks(state):
    ...
    all_dirs = [(-1, -1), (-1, 1), (1, -1), (1, 1)]
    forward  = [(-1, -1), (-1, 1)] if p == 0 else [(1, -1), (1, 1)]

    for r in range(8):
        for c in range(8):
            pc = state.board[r][c]
            if pc and pc.player == p:
                dirs = all_dirs if pc.is_dama else forward
                for dr, dc in dirs:
                    nr, nc = r + dr, c + dc
